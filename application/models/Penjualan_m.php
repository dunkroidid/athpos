...
class Penjualan_m extends CI_Model
{
    ...
    public function addItemPending($id, $qty, $subtotal, $harga, $id_jual)
    {
        $this->db->select("RIGHT (detil_penjualan.kode_detil_jual, 7) as kode_detil_jual", false);
        $this->db->order_by("kode_detil_jual", "DESC");
        $this->db->limit(1);
    ...
    public function hapusDetailPending($id)
    {
        $sql = "delete from detil_penjualan where id_detil_jual = '$id'";
        $this->db->query($sql);
    }
    ...
    public function simpaneditPenjualan()
    {
        date_default_timezone_set('Asia/Jakarta');
        ...
        $getDetil = $this->db->get_where('detil_penjualan', ['ID_DETIL_JUAL' => $id])->result_array();
        foreach ($getDetil as $dt) {
            $qty = $dt['QTY_JUAL'];
            $idBrg = $dt['ID_BARANG'];
            $getBrg = $this->db->get_where('barang', ['ID_BARANG' => $idBrg])->row_array();
            $stokBrg = $getBrg['STOK'];
        ...
        $this->db->set($data)->where('id_jual',$id)->update('penjualan');
